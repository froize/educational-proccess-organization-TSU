...
{
    public partial class Lecture_Halls : MetroFramework.Forms.MetroForm
    {
        DbEntities db;
        public Lecture_Halls(DbEntities bd)
        {
            InitializeComponent();
            db = bd;
        }
        ...
        {
            using (AddEditHalls frm = new AddEditHalls(new LectureHall()))
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        lectureHallBindingSource.Add(frm.LectureHallInfo);
                        db.LectureHall.Add(frm.LectureHallInfo);
        ...
        {
            LectureHall obj = lectureHallBindingSource.Current as LectureHall;
            if (obj != null)
            {
                using (AddEditHalls frm = new AddEditHalls(obj))
                {
                    if (frm.ShowDialog() == DialogResult.OK)
                    {
                        try
            ...
            if (MessageBox.Show("Вы удаляете запись. Вы уверены в этом?", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                for (int i = rows - 1; i >= 0; i--)
                {
                    if (metroGrid1.Rows[i].Selected)
                    {
                        try
                        {
                            db.LectureHall.Remove(metroGrid1.Rows[i].DataBoundItem as LectureHall);
                            lectureHallBindingSource.RemoveAt(metroGrid1.Rows[i].Index);
                            await db.SaveChangesAsync();
                        }
                        catch (Exception ex)
                        {
        ...
        private void Lecture_Halls_Load(object sender, EventArgs e)
        {
