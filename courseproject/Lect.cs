...
{
    public partial class Lect : MetroFramework.Forms.MetroForm
    {
        DbEntities db;
        public Lect(DbEntities bd)
        {
            InitializeComponent();
            db = bd;
        }
        ...
        {
            using (AddEditLecturers frm = new AddEditLecturers(new Lecturer()))
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        db.Lecturer.Add(frm.LecturerInfo);
        ...
        private async void mtEdit_Click(object sender, EventArgs e)
        {
            ...
            if (obj != null)
            {
                using (AddEditLecturers frm = new AddEditLecturers(obj))
                {
                    if (frm.ShowDialog() == DialogResult.OK)
                    {
                        try
            ...
            if (MessageBox.Show("Вы удаляете запись. Вы уверены в этом?", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                for (int i = rows - 1; i >= 0; i--)
                {
                    if (metroGrid1.Rows[i].Selected)
                    {
                        try
                        {
                            db.Lecturer.Remove(metroGrid1.Rows[i].DataBoundItem as Lecturer);
                            lecturerBindingSource.RemoveAt(metroGrid1.Rows[i].Index);
                            await db.SaveChangesAsync();
                        }
                        catch (Exception ex)
                        {
        ...
        private void Lect_Load(object sender, EventArgs e)
        {
            lecturerBindingSource.DataSource = db.Lecturer.ToList();
        }
    }
}
